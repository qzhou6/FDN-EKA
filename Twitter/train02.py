...
from mymodel import test_dataloader,train_dataloader,FeatureMapper,CLIPWithFeatureMapping,CustomClassifier
...
for epoch in range(num_epochs):
    ...
    for tweetIds, tweetTexts, tweetText_features, imageIds, img_features, external_features, labels in progress_bar:
        ...
        for i in range(batch_size):
            ...
            if similarity_score > 0 and fused_similarity_score > 0:
                # Both scores are greater than 0, predict real news (label 0)
                manual_predictions.append(0)
            else:
                # Both scores are less than or equal to 0, predict fake news (label 1)
        ...
        # 更新进度条的描述
        progress_bar.set_postfix({"accuracy": sum([1 for x, y in zip(all_labels, all_preds) if x == y]) / total_samples})
...
#                     if similarity_score > 0.95 and fused_similarity_score > 0.95:
#                         # Both scores are greater than 0.5, predict real news (label 0)
#                         manual_predictions.append(0)
#                     elif similarity_score <= -2 and fused_similarity_score <= -2:
#                         # Both scores are less than or equal to 0.5, predict fake news (label 1)
#                         manual_predictions.append(1)
#                     else:
#                         # Further model-based prediction is required
#                         outputs = model(mapped_image_features_orin[i:i+1], mapped_ex_features_ori[i:i+1], mapped_text_features_orin[i:i+1])
